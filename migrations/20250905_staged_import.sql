...
CREATE TABLE IF NOT EXISTS import_batches (
  ...
  source TEXT NOT NULL,            -- 'plaid'
  item_id TEXT,
  account_ids TEXT,                -- JSON array string
  start_date TEXT,
  end_date TEXT,
  status TEXT DEFAULT 'raw',       -- raw | suggested | committed | discarded
...
CREATE TABLE IF NOT EXISTS import_raw (
  ...
  name TEXT,                       -- Plaid name
  merchant_name TEXT,
  amount REAL,
  pending INTEGER,
  currency TEXT,
  original_json TEXT               -- full txn JSON
);
